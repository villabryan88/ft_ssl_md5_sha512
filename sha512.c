...
/*   Updated: 2019/04/23 13:50:46 by bvilla           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void				make_word_array(unsigned char *chunk, unsigned long *w)
{
	...
	while (i < 79)
	{
		s[0] = rtrot(w[i - 15], 1) ^ rtrot(w[i - 15], 8) ^ (w[i - 15] >> 7);
		s[1] = rtrot(w[i - 2], 19) ^ rtrot(w[i - 2], 61) ^ (w[i - 2] >> 6);
									...
									unsigned long  i)
{
	...
	s[1] = rtrot(E, 14) ^ rtrot(E, 18) ^ rtrot(E, 41);
	ch = (E & F) ^ ((~E) & G);
	tmp[0] = H + s[1] + ch + g_k[i] + w[i];
	s[0] = rtrot(A, 28) ^ rtrot(A, 34) ^ rtrot(A, 39);
	maj = (A & B) ^ (A & C) ^ (B & C);
	tmp[1] = s[0] + maj;
	H = G;
	G = F;
								...
								unsigned long curr_digest[8])
{
	...
	ft_memcpy(new_digest, curr_digest, sizeof(new_digest));
	i = 0;
	while (i < 80)
	{
